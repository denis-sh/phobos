...
*/
package void setInitialState(T)(ref T t)
{
    alias Unqual!T U;

    // If there is no elaborate assign and no both elaborate copy constructor
    // and elaborate destructor `t = T.init` has no side effects.
    static if(hasElaborateAssign!T ||
              (hasElaborateCopyConstructor!T && hasElaborateDestructor!T) ||
              (!isAssignable!T && !isAssignable!U))
    {
        import core.stdc.string;
...
unittest
{
    ...
    static assert(!__traits(compiles, setInitialState(5))); // doesn't accept rvalue
...
unittest // const
{
    ...
    setInitialState(*cast(S*) &i);
    assert(i == 1); i = 0;

    setInitialState(*cast(const S*) &i);
    assert(i == 1); i = 0;
}


/** Templated struct postblit caller

Faster and convenient replacement for $(D typeid(T).postblit(&t)).
*/
package void callCopyConstructors(T)(ref T t)
{
...
{
    int i = -1;
    callCopyConstructors(i); // no-op for non-elaborate types

    static assert(!__traits(compiles, callCopyConstructors(5))); // doesn't accept rvalue
    ...
    S s = void; s.i = -1;
    callCopyConstructors(s);
    checkLog(-1);

    S[3][2][1] sArr = void;
    foreach(j, ref el; *cast(S[6]*) sArr.ptr)
        el.i = j;
    callCopyConstructors(sArr);
...
/** Templated struct destructor caller.
